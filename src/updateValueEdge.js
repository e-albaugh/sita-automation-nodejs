...
export async function startQualityGate(lwSSOCookieValue) {
    const payloadQualityGateData = {
        phase: {
            type: "phase",
            id: "phase.process_quality_gate.inprogress"
        },
        id: `${process.env.VALUEEDGE_QUALITY_GATE_ID}`
    }
    try {
        // Send the PUT request with the payload
        const response = await client.put(
            `${process.env.VALUEEDGE_API_URL}/processes/${process.env.VALUEEDGE_QUALITY_GATE_ID}`,
            payloadQualityGateData,
            {
                headers: {
                    'Content-Type': 'application/json',
                    'LWSSO_COOKIE_KEY': lwSSOCookieValue,
        ...
        console.log("Started Quality Gate:", response.data);
        return response.data;
    } catch (error) {
        console.error("Error starting Quality Gate:", error.message);
        throw error;
    }
}